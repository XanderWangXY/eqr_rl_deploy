...
#ifndef GO2_POLICY_RUNNER_HPP_
#define GO2_POLICY_RUNNER_HPP_
...
public:
    Go2PolicyRunner(std::string policy_name):PolicyRunnerBase(policy_name){
        policy_path_ = GetAbsPath()+"/../policy/go2_fan_imp.pt";
        obs_total_dim_ = obs_dim_ + obs_his_num_*obs_dim_;
        dof_pos_default_.setZero(12);
        dof_pos_default_ << 0.0, 0.8, -1.5,
                            0.0, 0.8, -1.5,
                            0.0, 0.8, -1.5,
                            0.0, 0.8, -1.5;
    ...
    RobotAction GetRobotAction(const RobotBasicState& ro){
        ...
        //     joint_pos_diff_reordered[i] = (ro.joint_pos - dof_pos_default_)[original_idx];
        // }
        ...
        // for(int i = 0; i < 12; ++i) {
        //     int original_idx = new_order[i];
        //     joint_vel_reordered[i] = (0.05 * ro.joint_vel)[original_idx];
        // }

        current_obs_.setZero(obs_dim_);
        current_obs_ << 0.25*ro.base_omega,
                        projected_gravity,
                        cmd_vel,
                        ro.joint_pos - dof_pos_default_,  // 替换为调整后的向量
                        0.05 * ro.joint_vel,       // 替换为调整后的向量
                        // last_dof_pos2_, 
                        // last_dof_pos1_,
                        // last_dof_pos0_,  // 72
                        // 0.1*last_dof_vel1_,
        ...
        Eigen::MatrixXf temp = current_obs_.transpose();
